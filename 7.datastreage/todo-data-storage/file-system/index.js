...
const {readdir, readFile, writeFile, unlik} = require('fs').promises;

exports.fetchAll = async () =>{
    // 同一ディレクトリ内にあるファイルすべて取得
...
exports.update = async (id, update) => {
    ...
    return readFile(fileName, 'utf8').then(
        ...
        // ファイルが存在しない場合nullを返す、それ以外はそのままエラー
        err => err.code === 'ENOENT' ? null : Promise.reject(err);
    )
}

exports.remove = id => unlik.(`${__dirname}/${id}.json`)
    .then(
        () => id,
        // ファイルが存在しない場合nullを返す、それ以外はそのままエラー
        err => err.code === 'ENOENT' ? null : Promise.reject(err);
